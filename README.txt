...
Develop a running service that can make API Web service calls into twitter (to handle tweets) and flickr (to handle images). The service needs to have the ability to parse chess commands, check command syntax, handle improper commands, and process chess moves handling both valid and invalid moves. The service will also dynamically generate a new gameboard image.
    ...
    Get the current game board image URL and download image
    Perform gameboard image blob analysis to determine game state
...
3.0 Functional Requirements
    ...
    The application shall adhere to the rules of chess as defined by Reference 1.
    The application shall be able to interpret Forsyth–Edwards Notation (FEN) which is a method for describing a particular board position of a chess game. Reference 2.
    The chess pieces shall accurately reflect their rank through image and movement. Algebraic notation (or AN) is a method for recording and describing the moves in a game of chess. Reference 3.
